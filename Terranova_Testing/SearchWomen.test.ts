...
async function performTest() { 
  ...
  await searchInput.sendKeys('women');

  // Step 4: Execute the keyboard action to submit by pressing the 'Enter' key
  await searchInput.sendKeys(Key.RETURN);

  // Step 5: Check if women’s clothing items are displayed (you need to implement this check based on your webpage structure)
  const expectedUrl = 'https://www.terranovastyle.com/ba_bo/catalogsearch/result/?q=women';
  const isWomenOpen = await driver.wait(until.urlIs(expectedUrl), 20000);

  if (isWomenOpen) {
    console.log('Test passed: Women\'s section page is opened.');
  } else {
    console.error('Test failed: Women\'s section page is not opened.');
  }
